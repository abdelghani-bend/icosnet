...
class SaleOrder(models.Model):
    ...
    def _add_client_whmcs(self):
        ...
        custom_fields = [{"id":1, "value": 'Registre Commerce: ' + partner.number_registre },
            { "id":2, "value": base64.b64encode(('NIS: '+partner.number_carte_fiscale).encode()) },
            {"id":6, "value": base64.b64encode(('NIF: '+partner.number_nif).encode())},
            { "id":20, "value":   base64.b64encode(('Commercial: '+ partner.lastname + ' ' + partner.firstname).encode())}]
        # b64_custom_fields = base64.b64encode(json.dumps(custom_fields).encode()).decode()
    ...
    def _check_client_exists_whmcs(self):
        ...
        payload = {
            ...
            'clientid': self.partner_id.whmcs_clientid,
            'responsetype': 'json'
        }
